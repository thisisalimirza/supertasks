...
export function registerTaskHandlers(getMainWindow: () => BrowserWindow | null) {
  ...
  ipcMain.handle('onboarding:getStatus', () => db.isOnboardingCompleted())
  ipcMain.handle('onboarding:complete', (_, withDemoData: boolean) => db.completeOnboarding(withDemoData))
}
